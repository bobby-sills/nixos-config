...
require("pref-by-location"):setup({})
    ...
    keymap = {
      mgr.prepend_keymap = [
        ...
        { on = [ "g" "G" ]; run = ''shell "if mountpoint -q ~/gdrive; then systemctl --user stop rclone-gdrive; else systemctl --user start rclone-gdrive; fi"''; desc = "Toggle Google Drive mount"; }
